...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight (\a b -> bindOptional (\e -> mapOptional (e:.) b) a) (Full Nil)
...
-- | Tests all QuickCheck propositions in file
runListTests :: IO Bool
runListTests = $quickCheckAll
